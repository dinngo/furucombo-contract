...
contract HUniswapV2 is HandlerBase {
        ...
        )
    {
        ...
        address pair = UniswapV2Library.pairFor(router.factory(), token, router.WETH());
        _updateToken(pair);
    }
    ...
    function removeLiquidityETH(
        ...
        // Add liquidity ETH
        try router.removeLiquidityETH(token, liquidity, amountTokenMin, amountETHMin, address(this), now + 1) returns (
            uint256 ret1,
            uint256 ret2
        ) {
            amountToken = ret1;
    ...
    function removeLiquidity(
        ...
        // Add liquidity ETH
        try router.removeLiquidity(tokenA, tokenB, liquidity, amountAMin, amountBMin, address(this), now + 1) returns (
            uint256 ret1,
            uint256 ret2
        ) {
            amountA = ret1;
    ...
    function swapExactETHForTokens(
        ...
        if (path.length < 2) _revertMsg('swapExactETHForTokens', 'invalid path');
        address tokenOut = path[path.length - 1];

        // Get uniswapV2 router
        IUniswapV2Router02 router = IUniswapV2Router02(UNISWAPV2_ROUTER);
        try router.swapExactETHForTokens{value: value}(amountOutMin, path, address(this), now + 1) returns (
    ...
    function swapETHForExactTokens(
        ...
        if (path.length < 2) _revertMsg('swapETHForExactTokens', 'invalid path');
        address tokenOut = path[path.length - 1];

        // Get uniswapV2 router
        IUniswapV2Router02 router = IUniswapV2Router02(UNISWAPV2_ROUTER);

        try router.swapETHForExactTokens{value: value}(amountOut, path, address(this), now + 1) returns (
            uint256[] memory amounts
        ) {
            amount = amounts[0];
        } catch Error(string memory reason) {
    ...
    function swapExactTokensForETH(
        ...
        if (path.length < 2) _revertMsg('swapExactTokensForETH', 'invalid path');
        ...
        try router.swapExactTokensForETH(amountIn, amountOutMin, path, address(this), now + 1) returns (
            uint256[] memory amounts
        ) {
            amount = amounts[amounts.length - 1];
        } catch Error(string memory reason) {
    ...
    function swapTokensForExactETH(
        ...
        if (path.length < 2) _revertMsg('swapTokensForExactETH', 'invalid path');
        address tokenIn = path[0];
        ...
        try router.swapTokensForExactETH(amountOut, amountInMax, path, address(this), now + 1) returns (
            uint256[] memory amounts
        ) {
            amount = amounts[0];
        } catch Error(string memory reason) {
    ...
    function swapExactTokensForTokens(
        ...
        if (path.length < 2) _revertMsg('swapExactTokensForTokens', 'invalid path');
        address tokenIn = path[0];
        address tokenOut = path[path.length - 1];
        ...
        try router.swapExactTokensForTokens(amountIn, amountOutMin, path, address(this), now + 1) returns (
            uint256[] memory amounts
        ) {
            amount = amounts[amounts.length - 1];
        } catch Error(string memory reason) {
    ...
    function swapTokensForExactTokens(
        ...
        if (path.length < 2) _revertMsg('swapTokensForExactTokens', 'invalid path');
        address tokenIn = path[0];
        address tokenOut = path[path.length - 1];
        ...
        try router.swapTokensForExactTokens(amountOut, amountInMax, path, address(this), now + 1) returns (
            uint256[] memory amounts
        ) {
            amount = amounts[0];
        } catch Error(string memory reason) {
