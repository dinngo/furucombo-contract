...
contract HYVault is HandlerBase {
    ...
    function deposit(address vault, uint256 _amount) external payable returns (uint256) {
        IYVault yVault = IYVault(vault);
        uint256 beforeYTokenBalance = IERC20(address(yVault)).balanceOf(address(this));
        ...
        uint256 afterYTokenBalance = IERC20(address(yVault)).balanceOf(address(this));

        _updateToken(address(yVault));
        return afterYTokenBalance.sub(beforeYTokenBalance);
    }

    function depositETH(uint256 value, address vault) external payable returns (uint256) {
        IYVault yVault = IYVault(vault);
        uint256 beforeYTokenBalance = IERC20(address(yVault)).balanceOf(address(this));
        try yVault.depositETH{value: value}() {} catch Error(string memory reason) {
    ...
    function withdraw(address vault, uint256 _shares) external payable returns (uint256) {
        IYVault yVault = IYVault(vault);
        address token = yVault.token();
    ...
    function withdrawETH(address vault, uint256 _shares) external payable returns (uint256) {
