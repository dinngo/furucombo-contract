...
const { DAI_TOKEN, DAI_PROVIDER, BAT_TOKEN, BAT_PROVIDER, USDT_TOKEN, USDT_PROVIDER } = require('./utils/constants');
const { evmRevert, evmSnapshot, profileGas, getHandlerReturn } = require('./utils/utils');
...
contract('Funds', function([_, user, someone]) {
  ...
  before(async function() {
    ...
    await this.registry.register(this.hFunds.address, utils.asciiToHex('Funds'));
  });
  ...
  describe('check slippage', function() {
    ...
    it('normal', async function() {
      ...
      const to = this.hFunds.address;
      const data = abi.simpleEncode('checkSlippage(address[],uint256[])', token, value);
