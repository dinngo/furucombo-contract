...
contract('Liquidity', function ([_, deployer, user1, user2]) {
    ...
    describe('Add', function () {
        beforeEach(async function () {
            await this.token.transfer(this.proxy.address, ether('100'), { from: user1 });
        });
        ...
        it('normal', async function () {
            ...
            const data = [
                abi.simpleEncode('addLiquidity(uint256,address,uint256):(uint256)', value[0], tokenAddress, ether('100'))
            ];
            const deadline = (await latest()).add(new BN('100'));
            await this.token.approve(this.swap.address, ether('1000'), { from: user1 });
    ...
    describe('Remove', function () {
        ...
        beforeEach(async function () {
            ...
            await this.token.approve(this.swap.address, ether('1000'), { from: user2 });
        ...
        it('normal', async function () {
            ...
            const daiUser2Delta = (await this.token.balanceOf.call(user2)).sub(daiUser2);
