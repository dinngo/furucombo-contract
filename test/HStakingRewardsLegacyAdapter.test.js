...
contract(
  'StakingRewardsLegacyAdapter - Handler',
  function ([_, user, someone]) {
    let id;
    let balanceUser;
    /// st = stakingToken
    /// rt = rewardToken
    ...
    describe('Stake', function () {
      ...
      it('stake for msg.sender', async function () {
        ...
        // Check proxy balance
        expect(await this.st.balanceOf(this.proxy.address)).to.be.bignumber
          .zero;
    ...
    describe('Withdraw', function () {
      ...
      it('authorized', async function () {
        ...
        await this.adapter.setApproval(this.proxy.address, true, {
          from: user,
        });
        ...
        expect(stBalanceUserAfter.sub(stBalanceUser)).to.be.bignumber.eq(
          sValue
        );
        expect(await balanceUser.delta()).to.be.bignumber.eq(ether('0'));
        profileGas(receipt);
    ...
    describe('Exit', function () {
      ...
      it('authorized', async function () {
        ...
        expect(await this.st.balanceOf(this.proxy.address)).to.be.bignumber
          .zero;
        expect(await this.rt.balanceOf(this.proxy.address)).to.be.bignumber
          .zero;
        expect(await balance.current(this.proxy.address)).to.be.bignumber.zero;
        // Verify user staked amount has decreased
        expect(stakingUserAfter.sub(stakingUser)).to.be.bignumber.eq(
