const avalanche = require('../../deploy/utils/addresses_avalanche');

module.exports = {
  NATIVE_TOKEN_ADDRESS: '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE',
  NATIVE_TOKEN_DECIMAL: 18,

  /* Wrapped Native Token */
  WRAPPED_NATIVE_TOKEN: avalanche.WRAPPED_NATIVE_TOKEN,
  AWRAPPED_NATIVE_V3_TOKEN: '0x6d80113e533a2C0fe82EaBD35f1875DcEA89Ea97',
  AWRAPPED_NATIVE_V3_DEBT_VARIABLE:
    '0x4a1c3aD6Ed28a636ee1751C69071f6be75DEb8B8',

  /* WETH */
  WETH_TOKEN: '0x49D5c2BdFfac6CE2BFdB6640F4F80f226bc10bAB',
  AWETH_V3_TOKEN: '0xe50fA9b3c56FfB159cB0FCA61F5c9D750e8128c8',
  AWETH_V3_DEBT_STABLE: '0xD8Ad37849950903571df17049516a5CD4cbE55F6',
  AWETH_V3_DEBT_VARIABLE: '0x0c84331e39d6658Cd6e6b9ba04736cC4c4734351',

  /* DAI */
  DAI_TOKEN: '0xd586E7F844cEa2F87f50152665BCbc2C279D8d70',
  ADAI_V3_TOKEN: '0x82E64f49Ed5EC1bC6e43DAD4FC8Af9bb3A2312EE',
  ADAI_V3_DEBT_STABLE: '0xd94112B5B62d53C9402e7A60289c6810dEF1dC9B',
  ADAI_V3_DEBT_VARIABLE: '0x8619d80FB0141ba7F184CbF22fd724116D9f7ffC',

  /* WBTC */
  WBTC_TOKEN: '0x50b7545627a5162F82A992c33b87aDc75187B218',

  /* LINK */
  LINK_TOKEN: '0x5947BB275c521040051D82396192181b413227A3',

  /* USDT */
  USDT_TOKEN: '0x9702230A8Ea53601f5cD2dc00fDBc13d4dF4A8c7',

  /* USDC */
  USDC_TOKEN: '0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E',
  AUSDC_V3_TOKEN: '0x625E7708f30cA75bfd92586e17077590C60eb4cD',
  AUSDC_V3_DEBT_STABLE: '0x307ffe186F84a3bc2613D1eA417A5737D69A7007',
  AUSDC_V3_DEBT_VARIABLE: '0xFCCf3cAbbe80101232d343252614b6A3eE81C989',

  /* WAVAX */
  WAVAX_TOKEN: avalanche.WRAPPED_NATIVE_TOKEN,
  AWAVAX_V3_TOKEN: '0x6d80113e533a2C0fe82EaBD35f1875DcEA89Ea97',
  AWAVAX_V3_DEBT_STABLE: '0xF15F26710c827DDe8ACBA678682F3Ce24f2Fb56E',
  AWAVAX_V3_DEBT_VARIABLE: '0x4a1c3aD6Ed28a636ee1751C69071f6be75DEb8B8',

  /* COMP */
  COMP_TOKEN: '0xc3048E19E76CB9a3Aa9d77D8C03c29Fc906e2437',

  /* STG */
  STG_TOKEN: avalanche.STARGATE_TOKEN,

  /* AAVE Interest Rate Mode */
  AAVE_RATEMODE: { NODEBT: 0, STABLE: 1, VARIABLE: 2 },

  /* Services */
  AAVEPROTOCOL_V3_PROVIDER: avalanche.AAVEPROTOCOL_V3_PROVIDER,
  JOE_FACTORY: '0x9Ad6C38BE94206cA50bb0d90783181662f0Cfa10',
  STARGATE_PARTNER_ID: avalanche.STARGATE_PARTNER_ID,
  STARGATE_ROUTER: avalanche.STARGATE_ROUTER,
  STARGATE_ROUTER_ETH: avalanche.STARGATE_ROUTER_ETH,
  STARGATE_FACTORY: avalanche.STARGATE_FACTORY,
  STARGATE_WIDGET_SWAP: avalanche.STARGATE_WIDGET_SWAP,
  STARGATE_POOL_USDC: '0x1205f31718499dBf1fCa446663B532Ef87481fe1',
  STARGATE_DESTINATION_CHAIN_ID: 101, // Ethereum
  STARGATE_USDC_TO_DISALLOW_TOKEN_ID: 3, // DAI
  STARGATE_MULTISIG: '0x2b065946d41adf43bbc3baf8118ae94ed19d7a40',
  LAYERZERO_ENDPOINT: '0x3c2269811836af69497E5F486A85D7316753cf62',

  /* Event Signature */
  RecordHandlerResultSig:
    '0x90c726ff5efa7268723ee48df835144384bc0f012e89750782886764b5e54f16',

  // Handler Type
  HANDLER_TYPE: { TOKEN: 0, CUSTOM: 1, OTHERS: 2 },

  // Fee
  STORAGE_KEY_MSG_SENDER:
    '0xb2f2618cecbbb6e7468cc0f2aa43858ad8d153e0280b22285e28e853bb9d453a',
  STORAGE_KEY_CUBE_COUNTER:
    '0xf9543f11459ccccd21306c8881aaab675ff49d988c1162fd1dd9bbcdbe4446be',
  STORAGE_KEY_FEE_RATE:
    '0x142183525227cae0e4300fd0fc77d7f3b08ceb0fd9cb2a6c5488668fa0ea5ffa',
  STORAGE_KEY_FEE_COLLECTOR:
    '0x60d7a7cc0a45d852bd613e4f527aaa2e4b81fff918a69a2aab88b6458751d614',

  // Star NFT v4
  STAR_NFTV4: avalanche.FREE_PASS,
};
