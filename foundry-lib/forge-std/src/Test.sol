...
abstract contract Test is DSTest, Script {
    ...
    function deal(
        ...
        (, bytes memory balData) =
            token.call(abi.encodeWithSelector(0x70a08231, to));
        ...
        // update total supply
        if (adjust) {
            (, bytes memory totSupData) =
                token.call(abi.encodeWithSelector(0x18160ddd));
        ...
        returns (Tx1559 memory)
    {
        string memory deployData = vm.readFile(path);
        string memory key =
            string(abi.encodePacked(".transactions[", vm.toString(index), "]"));
        ...
        returns (Receipt[] memory)
    {
        ...
        RawReceipt[] memory rawReceipts =
            abi.decode(parsedDeployData, (RawReceipt[]));
        ...
        returns (Receipt memory)
    {
        string memory deployData = vm.readFile(path);
        string memory key =
            string(abi.encodePacked(".receipts[", vm.toString(index), "]"));
        bytes memory parsedDeployData = vm.parseJson(deployData, key);
